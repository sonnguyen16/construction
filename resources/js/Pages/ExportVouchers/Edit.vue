<template>
  <AdminLayout>
    <template #header>Chỉnh sửa phiếu xuất kho</template>
    <template #breadcrumb>Chỉnh sửa phiếu xuất kho</template>
...
const initialItems = props.exportVoucher.items.map((item) => ({
  id: item.id,
  product_id: item.product_id,
  quantity: item.quantity,
  export_price: formatCurrency(item.export_price)
}))

const form = useForm({
  code: props.exportVoucher.code,
  project_id: props.exportVoucher.project_id,
  export_date: formatDateInput(props.exportVoucher.export_date),
  customer_id: props.exportVoucher.customer_id || '',
  notes: props.exportVoucher.notes || '',
  items: initialItems
})
...
const addItem = () => {
  ...
  if (existingIndex >= 0) {
    ...
    form.items.push({
      ...
      export_price: currentItem.value.export_price
    })
  }
...
onMounted(() => {
  ...
  // Set khách hàng đã chọn
  if (form.customer_id) {
    window.$('#customer_id').inputpicker('val', form.customer_id)
  }
...
const submit = () => {
  ...
  const processedItems = form.items.map((item) => ({
    ...item,
    export_price: parseCurrency(item.export_price)
  }))

  // Gán lại vào form
  form.items = processedItems

  form.put(route('export-vouchers.update', props.exportVoucher.id), {
    onSuccess: () => {
      showSuccess('Phiếu xuất kho đã được cập nhật thành công.')
    }
  })
}
</script>
