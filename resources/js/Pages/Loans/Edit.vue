<template>
  <AdminLayout>
    <template #header>Chỉnh sửa khoản vay</template>
    <template #breadcrumb>Quản lý khoản vay / Chỉnh sửa khoản vay</template>
                  ...
                  <div class="form-group">
                    ...
                    <div v-if="loan.contract_file" class="mt-2">
                      ...
                      <a :href="'/storage/' + loan.contract_file" target="_blank" class="ml-2">
                        <i class="fas fa-file-download"></i> {{ loan.contract_file.split('/').pop() }}
                      </a>
                    </div>
                  </div>
                </div>
...
const form = useForm({
  ...
  contractor_id: props.loan.contractor_id,
...
onMounted(() => {
  ...
  // Đặt giá trị mặc định cho contractor_id
  if (props.loan.contractor_id) {
    window.$('#contractor_id').inputpicker('val', props.loan.contractor_id)
  }
  ...
  // Đặt giá trị mặc định cho project_id
  if (props.loan.project_id) {
    window.$('#project_id').inputpicker('val', props.loan.project_id)
  }
...
const submit = () => {
  ...
  form.post(route('loans.update', props.loan.id), {
    preserveScroll: true,
    onSuccess: () => {
      showSuccess('Cập nhật khoản vay thành công!')
    }
  })
}
</script>
